import { Grid, GridListTile, TextField } from '@material-ui/core'
...
class TableFilter extends React.Component {
  ...
  renderSelect(column, index) {
    ...
    return (
      <div key={index} className='section-filter'>
        <FormControl size='small' variant="outlined" key={index} className={classes.formControl}>
          <InputLabel id={`custom-filter-${column.name}`}>{column.label}</InputLabel>
          <Select
            fullWidth
            labelId={`custom-filter-${column.name}`}
  ...
  render() {
    ...
    return (
      <div className='block-filter wrap-main'>
